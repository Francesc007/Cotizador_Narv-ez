...
export function normalizeCotizacionEstatus(estatus) {
  const value = typeof estatus === "string" ? estatus.trim() : "";
  const lower = value.toLowerCase();

  if (lower === ESTATUS.CERRADA.toLowerCase()) return ESTATUS.CERRADA;
  if (lower === ESTATUS.PENDIENTE.toLowerCase()) return ESTATUS.PENDIENTE;
  // Legado: registros antiguos con "Enviada" se tratan como Pendiente.
  if (lower === "enviada") return ESTATUS.PENDIENTE;

  return value || ESTATUS.PENDIENTE;
}
