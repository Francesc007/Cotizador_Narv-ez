import { isQuoteInMexicoWeek } from "@/lib/dates/mexico";
...
/** Badge de eficiencia en el leaderboard: misma paleta que la planta del vendedor. */
export function getPlantaEficienciaClass(empresaId) {
  if (empresaId === EMPRESA_IDS.TEPEXI) {
    return "bg-[#103a6b]/12 text-[#103a6b] ring-1 ring-[#103a6b]/30";
  }

  if (empresaId === EMPRESA_IDS.NARVAEZ) {
    return "bg-[#ea580c]/12 text-[#ea580c] ring-1 ring-[#ea580c]/30";
  }
...
export function isQuoteInCurrentWeek(creadoAt) {
  return isQuoteInMexicoWeek(creadoAt);
}
